...
def test_openrouter_arun():

    async def arun():

        llm = Text(text="What is the capital of France ?")
        res = await llm.arun_openrouter()
...
def test_openrouter_stream():
    ...
    async def astream():
        ...
        async for res in llm.astream_openrouter():
            assert isinstance(res, str)

    asyncio.run(astream())


def test_openrouter_structured_extraction():
    ...
    async def astructured_extraction():
        ...
        res = await llm.astructured_extraction_openrouter(
...
def test_openrouter_astream_structured_extraction():
    ...
    async def arun():
        res = await llm.astream_structured_extraction_openrouter(
            output_class=ExtractedData,
        )
